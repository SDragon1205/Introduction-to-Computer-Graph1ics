...
GLuint Dissolveprogram, Modelprogram, Erect3program, Frag3program, Erect4program, Frag4program, Explodeprogram;
...
void demo() {
	if (model_f) {
		...
		if (who) {
			...
			M2 = glm::translate(M2, glm::vec3(near, -near, near));
			...
			if (explode_f) {
				...
				GLuint vecID = glGetUniformLocation(Explodeprogram, "explosion");
				glUniform1f(vecID, ex_sec);
				glUseProgram(0);
				DrawModel(Eevee, Explodeprogram, Eevee_VAO, Eevee_texture_ID, M2, GL_TRIANGLES);
			}
			else if (diss_f_e) {
		...
		if(who){//else {
			...
			// model matrix & transformation function to control model position
			glm::mat4 M1(base_M);
			M1 = glm::translate(M1, glm::vec3(0, 1.5, 0));
			if (sec < 1000 && sec > 500)
			{
				nnn2();
			}
			M1 = glm::translate(M1, glm::vec3(near, -near, near));
			M1 = glm::rotate(M1, glm::radians(angle), glm::vec3(0, 1, 0));
			...
			glm::vec4 front_line(0, 0, Umbreon->min_z, 1);
			...
			front_line.z += Umbreon_effect_t * sec;
			if (explode_f) {
				...
				//glUniform1f(vecID, ex_sec);
				//glUseProgram(0);
				//DrawModel(Umbreon, Explodeprogram, Umbreon_VAO, Umbreon_texture_ID, M1, GL_LINES_ADJACENCY);
			}
			else if (diss_f) {
			...
			else if (erect_f_u || frag_f) {
				if (front_line.z < Umbreon->max_z) {//(back_line.z < Umbreon->max_z) {
					// only create the "Goosebump" effect (I think it's look like pokemon erecting its hair) in a fixed range 
					//front_line = back_line + 0.5f * base_line_normal;
				}
				else {
